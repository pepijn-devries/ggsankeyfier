...
You can also position the infliction point of the curve. By default it's exactly
at the center in between the two connecting nodes. By setting the `curve_weight`
parameter to values close to zero, the infliction point will move to the left.
Values close to one will make it move to the right. You can set this parameter
for each individual stage (the same is true for the `slope` parameter).

```{r curve_weight, fig.width=6, fig.height=3}
p +
