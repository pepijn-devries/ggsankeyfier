# Authors and Citation

## Authors

- **Pepijn de Vries**. Author, maintainer, data contributor.
  [](https://orcid.org/0000-0002-7961-6646)

- **Gerjan Piet**. Data contributor.
  [](https://orcid.org/0000-0003-0702-1624)

- **Ruud Jongbloed**. Data contributor.
  [](https://orcid.org/0000-0002-9378-5382)

- **Anne Grundlehner**. Data contributor.
  [](https://orcid.org/0000-0003-3375-3511)

- **Jacqueline Tamis**. Data contributor.
  [](https://orcid.org/0000-0002-8206-5830)

## Citation

Source:
[`DESCRIPTION`](https://github.com/pepijn-devries/ggsankeyfier/blob/master/DESCRIPTION)

de Vries P (2025). *ggsankeyfier: Create Sankey and Alluvial Diagrams
Using 'ggplot2'*. R package version 0.1.8.0010,
<https://pepijn-devries.github.io/ggsankeyfier/>.

    @Manual{,
      title = {ggsankeyfier: Create Sankey and Alluvial Diagrams Using 'ggplot2'},
      author = {Pepijn {de Vries}},
      year = {2025},
      note = {R package version 0.1.8.0010},
      url = {https://pepijn-devries.github.io/ggsankeyfier/},
    }
